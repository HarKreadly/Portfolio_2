...
const About2 = () => {
  ...
  const handleNext = () => {
    if (currentIndex < personalStory.length - 1 && containerRef.current) {
      ...
      const targetY = containerRef.current.offsetTop + nextIndex * window.innerHeight;
  ...
  const handlePrev = () => {
    if (currentIndex > 0 && containerRef.current) {
      ...
      const targetY = containerRef.current.offsetTop + prevIndex * window.innerHeight;
      window.scrollTo({ top: targetY, behavior: 'smooth' });
    }
  };
      ...
      <div className="w-full h-full max-w-[1600px] mx-auto flex flex-col lg:flex-row px-8 md:px-16 mt-16 lg:mt-0 relative z-10 items-center justify-between gap-12 lg:gap-24">
        ...
        <div className="w-full lg:w-[60%] h-[50vh] md:h-[60vh] lg:h-[75vh] relative flex items-center mt-12 lg:mt-0">
                  ...
                  <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                    {isActive ? (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                           <Heart size={14} /> <span>{1350 + index * 5}</span>
                        </div>
                      </motion.div>
                    ) : (
                      <div className="text-white/80 font-serif text-3xl lg:text-4xl font-bold ml-auto opacity-70"
          ...
          <div className="absolute bottom-0 left-0 w-full flex text-[10px] text-gray-500 leading-relaxed tracking-wide">
             <div className="w-[15%] pr-4 hidden md:block border-t border-gray-200 dark:border-gray-800 pt-4">
                {currentIndex > 1 && (
                   <p className="line-clamp-4 pr-2">{personalStory[currentIndex - 2].description}</p>
                )}
             </div>
             <div className="w-[18%] pr-4 hidden md:block border-l border-t border-gray-200 dark:border-gray-800 pl-4 pt-4">
                {currentIndex > 0 && (
                   <p className="line-clamp-4 pr-2">{personalStory[currentIndex - 1].description}</p>
                )}
             </div>
        ...
        <div className="w-full lg:w-[35%] flex flex-col justify-center relative pl-0 lg:pl-12 h-full lg:h-[75%] pb-20 lg:pb-0">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <p className="text-gray-400 dark:text-gray-500 text-xs md:text-sm leading-loose max-w-sm ml-0 lg:ml-[5.5rem]">
                  Looking for inspiration for your soul project? Search for images based on colors, image formats, etc. Our brain is continually exposed to internal and external stimuli. Silence feels impossible, like emptying our spirit. <br/><br/>
                  <span className="font-bold text-gray-900 dark:text-gray-300 block mb-1">Details:</span>
                  <span className="opacity-80">{currentStory.items.join(' — ')}</span>
                </p>
              </motion.div>
            </AnimatePresence>
          </div>
          ...
          <div className="flex items-center gap-4 mt-12 lg:mt-auto border-t border-gray-200 dark:border-gray-800 pt-6">
             ...
             >
               <span className="text-[10px] tracking-[0.2em] font-bold mb-3 text-black dark:text-white">PREV</span>
               <div className="flex overflow-hidden text-gray-400">
                 <ChevronLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
                 <ChevronLeft size={16} className="-ml-2 group-hover:-translate-x-1 transition-transform" />
               </div>
             </button>
             ...
             >
               <span className="text-[10px] tracking-[0.2em] font-bold mb-3 text-black dark:text-white">NEXT</span>
               <div className="flex overflow-hidden text-gray-400">
